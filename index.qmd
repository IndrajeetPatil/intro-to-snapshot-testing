...
![](media/logos_combined.webp){.absolute width="750" height="300"}
...
```{=html}
<iframe
  src="https://shinylive.io/r/app/#h=0&amp;code="
  title="Interactive Shiny app that converts kilograms to grams"
  aria-label="Interactive Shiny app that converts kilograms to grams"
  loading="lazy"
  style="width: 100%; height: 420px; border: 1px solid #dee2e6; border-radius: 8px; background: #ffffff;"
></iframe>
```
...
::: {style="font-size: 0.45em; text-align: center; margin-top: 0.2em;"}

Runs entirely in the browser via [Shinylive](https://posit-dev.github.io/r-shinylive/); the first load can take a few seconds.
...
Frameworks such as [`{golem}`](https://thinkr-open.github.io/golem/index.html) and [`{rhino}`](https://appsilon.github.io/rhino/) need no separate shinytest2 driver layer when they expose a package function that runs or returns an app.

. . .

Pass that function directly to `AppDriver$new()`. During local package development, shinytest2 runs it in a background process and loads the development package source for you.

:::{.callout-tip}

Installing the package before local testing is no longer required.
