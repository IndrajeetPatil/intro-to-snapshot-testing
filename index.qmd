...
## Native safety rails {.smaller}

`{testthat}` now handles several snapshot-management problems itself:

. . .

- **Duplicate file names** — reusing a name in `expect_snapshot_file()` produces a clear error instead of silently overwriting the first snapshot. This also protects wrappers such as `vdiffr::expect_doppelganger()`.

. . .

- **Dangling snapshots** — after a complete local test run, ordinary snapshot files that no test generated are deleted automatically. You don't need a manual orphan-cleanup step.

. . .

- **Unreviewed additions on CI** — a brand-new snapshot fails on CI instead of being accepted as the new reference.

:::{.callout-caution}

Variants are the exception: testthat cannot know every valid variant in advance, so you remain responsible for deleting variant directories that are no longer used.
