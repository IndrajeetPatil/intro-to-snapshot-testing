...
test_that("`unitConverter` app works", {
  # Don't run these tests on the CRAN machines
  skip_on_cran()
...
server <- function(input, output, session) {
  output$g <- renderText(
    # should be `"Weight (in g): "` instead
    paste0("Weight (in kg): ", input$kg * 1000)
  )
}
...
The final location of the tests and snapshots should look like the following for the two possible ways Shiny apps are included in a package.
...
The easiest solution is to copy the new snapshots to your local folder and then run `snapshot_review()`.
