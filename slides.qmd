...
If the function changes, the snapshots won't match the expected ones, and test will fail:
...
- Don't **blindly accept** snapshot changes: Resist formation of such a habit. `{testthat}` provides tools to make it extremely easy to review changes, so that is no excuse! 
...
If you are now feeling confident about your snapshot testing skills and want to contribute to open-source projects to practice these skills, here are some options.
