...
If tests fail even if you didn't change anything about your plotting function, it can be due to any of the following reasons:
...
For these reasons, snapshot tests for plots tend to be fragile and not run on CRAN machines by default.
...
`{shinytest2}` uses a Shiny app (how meta! 😅) to record your interactions with the app and generate snapshots of the application's state. Future behaviour of the app will be compared against these snapshots to check for any changes.

. . . 

:::{.callout-note}

- `{shinytest2}` itself will create a test file when you run `record_test()` in the app directory.

- The test script will be saved in a *subdirectory* of the app If your app is located in (e.g.) `inst/` folder, the test will be in `inst/my-app/tests/testthat/test-shinytest2.R`.
