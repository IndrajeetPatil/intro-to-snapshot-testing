...
then you should be excited to hear about *snapshot tests* (aka *golden tests*), which can relieve above-mentioned pain-points! 🤩
...
Note that we want to test that the graphical output *looks* as expected.

## Graphical snapshot test {.smaller}

We can use `expect_doppelganger()` from `{vdiffr}` to do this!
...
But if we change the behavior of underlying function (use different aesthetic properties for `geom_point()`), the actual snapshot output won't match the expected one, and the test will fail:
...
## How to peer review? {.smaller}

Despite snapshot tests making the expected outputs more human-readable, given a big enough change and complex enough output, it can be challenging to review changes to snapshots. 

How do you review such pull requests? Here two approaches I prefer.
