...
### Why does my test fail on a rerun?
...
<!-- Don't add . . .  here since it breaks the flow -->
...
- If a test is called `test-foo.R`, the snapshot will be saved to `test/testthat/_snaps/foo` folder.
...
For these reasons, the visual regression tests tend to be fragile and by default not run on CRAN machines.
...
`{shinytest2}` uses a Shiny app (how meta! 😅) to record your interactions with the app and generates snapshots of the application's state. Future behaviour of the app will be compared against these snapshots to check for any changes.

. . . 

:::{.callout-note}

- You don't need to create a test file; `{shinytest2}` will do that for you! You will just need to run `record_test()`.

- The test script will be saved in a *subdirectory* of the application.If your app is located in (e.g.) `inst/` folder, the test will be in `inst/my-app/tests/testthat/test-shinytest2.R`.

- There will be one `tests` folder inside every application folder.

- The snapshots are saved as `.png` file in `tests/testthat/_test-shinytest2/_snaps/{.variant}/shinytest2`. The `{.variant}` here corresponds to operating system and R version used to record tests. For example, `_snaps/windows-4.1/shinytest2`.

:::

## Example Shiny app {.smaller}

Let's say this app resides in the `inst/unitConverter` package directory.
...
Let's say, while updating the app, we make a mistake. This'd lead to a test failure.
...
server <- function(input, output, session) {
  output$g <- renderText(paste0("Weight (in kg): ", input$kg * 1000))
}
...
It's not all kittens and roses when it comes to snapshot testing. We will now see some issues you might run into while using them. 🤕
...
The easiest solution in such cases is to copy the new snapshots to your local folder and then run `snapshot_review()`.

:::{.callout-tip}

If your expected snapshot is called (e.g.) `foo.svg`, when the test fails, there will be a new snapshot file `foo.new.svg` in the same folder.

`snapshot_review()` compares these files to reveal how the output has changed.
...
How do you review pull requests with complex changes in snapshots? 
...
Use tools provided for code review by hosting platform (like GitHub).
...
Although this makes total sense, it means that you will miss out on anything but a breaking change from upstream dependency.

For example, if `{ggplot2}` (hypothetically) changes how the points look, you will completely miss out on this change until you *happen to* run your snapshot tests again locally or on CI/CD after `{ggplot2}` update.

Unit tests would alert to such changes because they are run on CRAN, and so you'll be informed to update the package when tests start failing.

The only solution I can think of to insure against this is to run such tests **daily** on CI/CD platforms (e.g. AppVeyor nightly builds).
...
- Don't use snapshot tests **for everything**: It is tempting to use them everywhere out of laziness. But they are sometimes inappropriate (e.g. when testing requires external benchmarking).

- Don't **blindly accept** snapshot changes: Resist formation of such a habit. `{testthat}` provides tools to make it extremely easy to review changes, so you have no excuses! 
...
If you are now feeling confident about your snapshot testing skills, if you want, you can contribute to open-source projects to practice these skills!

## Practice makes it perfect {.smaller}

These are only suggestions. Feel free to contribute to any project that you like! 🤝

:::{.callout-tip}

## Suggestions 

- See if any of the `{ggplot2}` [extensions](https://exts.ggplot2.tidyverse.org/gallery/) you like use snapshot tests for graphics. If not, you can add tests for key functions.
...
- `{shinytest2}` is the successor to `{shinytest}` package. Check out which packages currently use it for [testing Shiny apps](https://cran.r-project.org/web/packages/shinytest/index.html), and see if you can use `{shinytest2}` instead (see how-to [here](https://rstudio.github.io/shinytest2/articles/z-migration.html)).
