...
Exactly how you test Shiny apps in R package will depend on how the app is stored.
...
# Shiny app as `inst/` subdirectory 
...
- `{shinytest2}` itself will create a test file when you run `record_test()` in the app directory. The test script will be saved in a *subdirectory* of the app (`inst/my-app/tests/testthat/test-shinytest2.R`).
...
Note that currently your test scripts and results are in the `/inst` folder, but you would also want these tests to be run using `{testthat}`.

For this, we will need to write a driver script (`tests/testthat/test-inst-apps.R`).

```{.r}
library(shinytest2)

test_that("`unitConverter` app works", {
  # Don't run these tests on the CRAN machinws
  skip_on_cran()
...
You can use Shiny app provided by `{testthat2}` for comparing snapshots.
...
- [Testing](https://mastering-shiny.org/scaling-testing.html) chapter from *Mastering Shiny*
...
We will now see some issues you might run into while using them. 🤕
...
Although this makes sense, it means that you will miss out on anything but a breaking change from upstream dependency. E.g., if `{ggplot2}` (hypothetically) changes how the points look, you will miss out on this change until you *happen to* run your snapshot tests again locally or on CI/CD.
...
To see a more realistic usage of snapshot tests, you can study open-source test suites on your own. 
...
For a comprehensive collection of packages for unit testing in R, see [this](https://github.com/IndrajeetPatil/awesome-r-pkgtools/blob/main/README.md#unit-testing-) page.
